...
public class SafeIterableMap<K, V> implements Iterable<Map.Entry<K, V>> {
  ...
  private abstract static class ListIterator<K, V>
      implements Iterator<Map.Entry<K, V>>, SupportRemove<K, V> {
  ...
  static class AscendingIterator<K, V> extends ListIterator<K, V> {
    ...
    @Override Entry<K, V> forward(Entry<K, V> entry) {
      return entry.mNext;
    }

    @Override Entry<K, V> backward(Entry<K, V> entry) {
