...
public class Example {
  ...
  private static final Address CONTRACT_ADDRESS =
      new Address("8dbf4349cbeca08a02cc6b5b0862f9dd42c585b9");
  private static final Address RECEIVER_ADDR =
      new Address("62a5c1680554A61334F5c6f6D7dA6044b6AFbFe8");
  ...
  private void run() {
    ...
    TransactionResultResponse call =
        this.ethereumApi.call((int) nonce, senderKey, gasPrice, gasLimit, CONTRACT_ADDRESS,
            new Erc20Transfer(RECEIVER_ADDR.getValue(), 1).encode())
            .toBlocking()
            .first();
    ...
    BalanceResponse balance = this.ethereumApi.getBalance(RECEIVER_ADDR.getValue())
        .toBlocking()
        .first();
    BalanceResponse tokenBalance =
        this.ethereumApi.getTokenBalance(CONTRACT_ADDRESS.getValue(), RECEIVER_ADDR.getValue())
            .toBlocking()
            .first();
