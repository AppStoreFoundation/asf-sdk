...
import cm.aptoide.pt.web3j.abi.datatypes.Array;
import cm.aptoide.pt.web3j.abi.datatypes.Bool;
import cm.aptoide.pt.web3j.abi.datatypes.Bytes;
import cm.aptoide.pt.web3j.abi.datatypes.DynamicArray;
import cm.aptoide.pt.web3j.abi.datatypes.DynamicBytes;
import cm.aptoide.pt.web3j.abi.datatypes.Fixed;
import cm.aptoide.pt.web3j.abi.datatypes.FixedPointType;
import cm.aptoide.pt.web3j.abi.datatypes.Int;
import cm.aptoide.pt.web3j.abi.datatypes.IntType;
import cm.aptoide.pt.web3j.abi.datatypes.NumericType;
import cm.aptoide.pt.web3j.abi.datatypes.StaticArray;
import cm.aptoide.pt.web3j.abi.datatypes.Type;
import cm.aptoide.pt.web3j.abi.datatypes.Ufixed;
import cm.aptoide.pt.web3j.abi.datatypes.Uint;
import cm.aptoide.pt.web3j.abi.datatypes.Utf8String;
import cm.aptoide.pt.web3j.abi.datatypes.generated.Uint160;
...
public class TypeDecoder {
  ...
  static <T extends Type> int getSingleElementLength(String input, int offset, Class<T> type) {
    if (input.length() == offset) {
      return 0;
    } else if (DynamicBytes.class.isAssignableFrom(type) || Utf8String.class.isAssignableFrom(
        type)) {
  ...
  public static <T extends Array> T decode(String input, int offset,
      TypeReference<T> typeReference) {
    Class cls = ((ParameterizedType) typeReference.getType()).getRawType()
        .getClass();
  ...
  static <T extends NumericType> T decodeNumeric(String input, Class<T> type) {
    try {
      ...
      return type.getConstructor(BigInteger.class)
          .newInstance(numericValue);
  ...
  static <T extends NumericType> int getTypeLength(Class<T> type) {
    if (IntType.class.isAssignableFrom(type)) {
      ...
      String[] splitName = type.getSimpleName()
          .split(regex);
  ...
  static int decodeUintAsInt(String rawInput, int offset) {
    ...
    return decode(input, 0, Uint.class).getValue()
        .intValue();
  }
  ...
  static <T extends Bytes> T decodeBytes(String input, int offset, Class<T> type) {
    try {
      ...
      return type.getConstructor(byte[].class)
          .newInstance(bytes);
    } catch (NoSuchMethodException | SecurityException | InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException e) {
      throw new UnsupportedOperationException("Unable to create instance of " + type.getName(), e);
    }
   ...
   */
  @SuppressWarnings("unchecked") static <T extends Type> T decodeStaticArray(String input,
      int offset, TypeReference<T> typeReference, int length) {
  ...
  @SuppressWarnings("unchecked")
  private static <T extends Type> T instantiateStaticArray(TypeReference<T> typeReference,
      List<T> elements) {
    try {
  ...
  @SuppressWarnings("unchecked")
  static <T extends Type> T decodeDynamicArray(String input, int offset,
      TypeReference<T> typeReference) {
  ...
  private static <T extends Type> T decodeArrayElements(String input, int offset,
      TypeReference<T> typeReference, int length, BiFunction<List<T>, String, T> consumer) {
    ...
    try {
      ...
      if (Array.class.isAssignableFrom(cls)) {
        ...
        for (int i = 0, currOffset = offset; i < length; i++, currOffset +=
            getSingleElementLength(input, currOffset, cls) * MAX_BYTE_LENGTH_FOR_HEX_STRING) {
      ...
      throw new UnsupportedOperationException(
          "Unable to access parameterized type " + typeReference.getType()
              .getTypeName(), e);
    }
  }
}
