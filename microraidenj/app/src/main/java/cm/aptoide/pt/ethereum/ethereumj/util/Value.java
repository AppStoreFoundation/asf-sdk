...
public class Value {
  ...
  public Value(Object obj) {
    ...
    if (obj instanceof cm.aptoide.pt.ethereum.ethereumj.util.Value) {
      this.value = ((cm.aptoide.pt.ethereum.ethereumj.util.Value) obj).asObj();
    } else {
      this.value = obj;
    }
  }

  public static cm.aptoide.pt.ethereum.ethereumj.util.Value fromRlpEncoded(byte[] data) {

    if (data != null && data.length != 0) {
      cm.aptoide.pt.ethereum.ethereumj.util.Value v =
          new cm.aptoide.pt.ethereum.ethereumj.util.Value();
      v.init(data);
      return v;
    }
    return null;
  ...
  public cm.aptoide.pt.ethereum.ethereumj.util.Value withHash(byte[] hash) {
    sha3 = hash;
    return this;
  }
  ...
  public cm.aptoide.pt.ethereum.ethereumj.util.Value get(int index) {
    if (isList()) {
      // Guard for OutOfBounds
      if (asList().size() <= index) {
        return new cm.aptoide.pt.ethereum.ethereumj.util.Value(null);
      }
      if (index < 0) {
        throw new RuntimeException("Negative index not allowed");
      }
      return new cm.aptoide.pt.ethereum.ethereumj.util.Value(asList().get(index));
    }
  ...
  public void decode() {
    if (!this.decoded) {
      this.value = cm.aptoide.pt.ethereum.ethereumj.util.RLP.decode(rlp, 0)
          .getDecoded();
      this.decoded = true;
    }
  }
  ...
  public String toString() {
    ...
    if (isList()) {
      ...
      if (list.length == 2) {
        ...
        cm.aptoide.pt.ethereum.ethereumj.util.Value key =
            new cm.aptoide.pt.ethereum.ethereumj.util.Value(list[0]);
        ...
        cm.aptoide.pt.ethereum.ethereumj.util.Value val =
            new cm.aptoide.pt.ethereum.ethereumj.util.Value(list[1]);
      ...
      for (int i = 0; i < list.length; ++i) {
        cm.aptoide.pt.ethereum.ethereumj.util.Value val =
            new cm.aptoide.pt.ethereum.ethereumj.util.Value(list[i]);
  ...
  public int countBranchNodes() {
    ...
    if (this.isList()) {
      ...
      for (Object obj : objList) {
        i += (new cm.aptoide.pt.ethereum.ethereumj.util.Value(obj)).countBranchNodes();
      }
      return i;
