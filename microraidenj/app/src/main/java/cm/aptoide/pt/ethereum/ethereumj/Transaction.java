 ...
 */
package cm.aptoide.pt.ethereum.ethereumj;

import cm.aptoide.pt.ethereum.ethereumj.crypto.ECKey;
import cm.aptoide.pt.ethereum.ethereumj.crypto.ECKey.ECDSASignature;
import cm.aptoide.pt.ethereum.ethereumj.crypto.ECKey.MissingPrivateKeyException;
...
import static cm.aptoide.pt.ethereum.ethereumj.util.ByteUtil.EMPTY_BYTE_ARRAY;
import static cm.aptoide.pt.ethereum.ethereumj.util.ByteUtil.ZERO_BYTE_ARRAY;
...
public class Transaction {
  ...
  private static final Logger logger =
      Logger.getLogger(cm.aptoide.pt.ethereum.ethereumj.Transaction.class.getName());
   ...
   * Use {@link cm.aptoide.pt.ethereum.ethereumj.Transaction#Transaction(byte[], byte[], byte[], * byte[], byte[], byte[], Integer)} constructor instead
  ...
  /**
   * Warning: this transaction would not be protected by replay-attack protection mechanism
   * Use {@link cm.aptoide.pt.ethereum.ethereumj.Transaction#Transaction(byte[], byte[], byte[], * byte[], byte[], byte[], byte[], byte[], byte, Integer)}
  ...
  /**
   * @deprecated Use {@link cm.aptoide.pt.ethereum.ethereumj.Transaction#createDefault(String, * BigInteger, BigInteger, Integer)} instead
   */
  public static cm.aptoide.pt.ethereum.ethereumj.Transaction createDefault(String to,
      BigInteger amount, BigInteger nonce) {
    return create(to, amount, nonce, DEFAULT_GAS_PRICE, DEFAULT_BALANCE_GAS);
  }

  public static cm.aptoide.pt.ethereum.ethereumj.Transaction createDefault(String to,
      BigInteger amount, BigInteger nonce, Integer chainId) {
    return create(to, amount, nonce, DEFAULT_GAS_PRICE, DEFAULT_BALANCE_GAS, chainId);
  }

  /**
   * @deprecated use {@link cm.aptoide.pt.ethereum.ethereumj.Transaction#create(String, BigInteger, * BigInteger, BigInteger, BigInteger, Integer)} instead
   */
  public static cm.aptoide.pt.ethereum.ethereumj.Transaction create(String to, BigInteger amount,
      BigInteger nonce, BigInteger gasPrice, BigInteger gasLimit) {
    return new cm.aptoide.pt.ethereum.ethereumj.Transaction(BigIntegers.asUnsignedByteArray(nonce),
  ...
  public static cm.aptoide.pt.ethereum.ethereumj.Transaction create(String to, BigInteger amount,
      BigInteger nonce, BigInteger gasPrice, BigInteger gasLimit, Integer chainId) {
    return new cm.aptoide.pt.ethereum.ethereumj.Transaction(BigIntegers.asUnsignedByteArray(nonce),
        BigIntegers.asUnsignedByteArray(gasPrice), BigIntegers.asUnsignedByteArray(gasLimit),
  ...
  @Override public boolean equals(Object obj) {
    ...
    if (!(obj instanceof cm.aptoide.pt.ethereum.ethereumj.Transaction)) return false;
    cm.aptoide.pt.ethereum.ethereumj.Transaction tx =
        (cm.aptoide.pt.ethereum.ethereumj.Transaction) obj;
