...
import static cm.aptoide.pt.ethereum.ethereumj.util.ByteUtil.byteArrayToInt;
import static cm.aptoide.pt.ethereum.ethereumj.util.ByteUtil.intToBytesNoLeadZeroes;
import static cm.aptoide.pt.ethereum.ethereumj.util.ByteUtil.isNullOrZeroArray;
import static cm.aptoide.pt.ethereum.ethereumj.util.ByteUtil.isSingleZero;
...
public class RLP {
  ...
  public static DecodeResult decode(byte[] data, int pos) {
    ...
    if (prefix == OFFSET_SHORT_ITEM) {  // 0x80
      ...
      int lenbytes =
          byteArrayToInt(copyOfRange(data, pos + 1, pos + 1 + lenlen)); // length of encoded bytes
