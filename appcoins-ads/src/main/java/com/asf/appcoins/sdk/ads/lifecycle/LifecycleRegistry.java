...
public class LifecycleRegistry extends Lifecycle {
   ...
   */
  @Deprecated public void markState(State state) {
  ...
  private boolean isSynced() {
    ...
    State eldestObserverState = mObserverMap.eldest()
        .getValue().mState;
    State newestObserverState = mObserverMap.newest()
        .getValue().mState;
  ...
  private State calculateTargetState(LifecycleObserver observer) {
    ...
    State parentState =
        !mParentStates.isEmpty() ? mParentStates.get(mParentStates.size() - 1) : null;
  ...
  @Override public void addObserver(LifecycleObserver observer) {
    ...
    while ((statefulObserver.mState.compareTo(targetState) < 0 && mObserverMap.contains(
        observer))) {
   ...
   */
  @SuppressWarnings("WeakerAccess") public int getObserverCount() {
  ...
  private void forwardPass(LifecycleOwner lifecycleOwner) {
    ...
    while (ascendingIterator.hasNext() && !mNewEventOccurred) {
      ...
      while ((observer.mState.compareTo(mState) < 0 && !mNewEventOccurred && mObserverMap.contains(
          entry.getKey()))) {
  ...
  private void sync() {
    ...
    while (!isSynced()) {
      ...
      if (mState.compareTo(mObserverMap.eldest()
          .getValue().mState) < 0) {
