...
import com.sdk.appcoins_adyen.encryption.ClientSideEncrypter;
...
public class Card {
  ...
  /**
   * @return masked card number if the number is already available and the number of digits is
   * longer than 13. Otherwise empty string.
   */
  ...
  public static final class Builder {
    ...
    public Card build() throws NullPointerException, IllegalStateException {
      ...
      require(card.number == null || card.number.matches("[0-9]{8,19}"),
          "number must be null or have 8 to 19 digits (inclusive).");
      require(card.cardHolderName == null || card.cardHolderName.length() > 0,
          "cardHolderName must be null or not empty.");
      require(card.cvc == null || (card.cvc.matches("[0-9]{3,4}")),
          "cvc must be null or have 3 to 4 digits.");
      require(card.expiryMonth == null || card.expiryMonth.matches("0?[1-9]|1[0-2]"),
          "expiryMonth must be null or between 1 and 12.");
      require(card.expiryYear == null || card.expiryYear.matches("20\\d{2}"),
