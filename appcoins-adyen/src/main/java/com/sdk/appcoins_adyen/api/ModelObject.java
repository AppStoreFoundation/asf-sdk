 ...
 * input from the Checkout API.
 * All model objects can be serialized and deserialized to and from a JSONObject using the
 * {@link Serializer} interface.
 * All model object also implement Parcelable to be sent as part of Extras in an Intent Bundle.
 *
 * <p/>
 * The classes extending {@link ModelObject} are data classes designed to work standalone or in
 * association with JSON libraries like GSON and Moshi.
 */
public abstract class ModelObject implements Parcelable {

  @SuppressWarnings("PMD.EmptyMethodInAbstractClassShouldBeAbstract") @Override
  public final int describeContents() {
    return ParcelUtils.NO_FILE_DESCRIPTOR;
  }

  /**
   * Interface that must be implemented and provided as a public SERIALIZER field that
   * serializes the to and from a JSONObject.
   *
  ...
  public interface Serializer<T extends ModelObject> {
     ...
     */
    JSONObject serialize(T modelObject);
