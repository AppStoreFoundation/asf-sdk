...
public class ClientSideEncrypter {
  ...
  public ClientSideEncrypter(String publicKeyString) throws EncrypterException {
    ...
    RSAPublicKeySpec pubKeySpec =
        new RSAPublicKeySpec(new BigInteger(keyComponents[1].toLowerCase(Locale.getDefault()), 16),
  ...
  public String encrypt(String plainText) throws EncrypterException {
    ...
    try {
      ...
      return String.format("%s%s%s%s%s%s", PREFIX, VERSION, SEPARATOR,
  ...
  private synchronized byte[] generateIV(int ivSize) {
    byte[] iv = new byte[ivSize];//generate random IV AES is always 16bytes, but in CCM mode this
    // represents the NONCE
