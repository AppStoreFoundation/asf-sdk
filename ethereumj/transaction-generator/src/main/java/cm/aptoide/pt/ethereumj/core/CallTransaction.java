 ...
 */
package cm.aptoide.pt.ethereumj.core;
...
import cm.aptoide.pt.ethereumj.solidity.SolidityType;
import cm.aptoide.pt.ethereumj.util.ByteUtil;
import cm.aptoide.pt.ethereumj.util.FastByteComparisons;
import cm.aptoide.pt.ethereumj.vm.LogInfo;

import static cm.aptoide.pt.ethereumj.crypto.HashUtil.sha3;
import static cm.aptoide.pt.ethereumj.solidity.SolidityType.IntType;
import static cm.aptoide.pt.ethereumj.util.ByteUtil.longToBytesNoLeadZeroes;
...
public class CallTransaction {
	...
	private final static ObjectMapper DEFAULT_MAPPER = new ObjectMapper().disable(
					DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES)
					.enable(DeserializationFeature.READ_UNKNOWN_ENUM_VALUES_AS_NULL);

	public static Transaction createRawTransaction(long nonce, long gasPrice, long gasLimit, String toAddress, long value, byte[] data) {
		Transaction tx = new Transaction(longToBytesNoLeadZeroes(nonce),
						...
						toAddress == null ? null : Hex.decode(toAddress), longToBytesNoLeadZeroes(value), data,
						null);
		return tx;
	}

	public static Transaction createCallTransaction(long nonce, long gasPrice, long gasLimit, String
          toAddress, long value, Function callFunc, Object... funcArgs) {
	...
	public static class Function {
		...
		public static Function fromSignature(String funcName, String[] paramTypes, String[]
            resultTypes) {
	...
	public static class Contract {
		...
		public Invocation parseInvocation(byte[] data) {
			...
			if (function == null) throw new RuntimeException("Can't find function/event by it " +
              "signature");
		...
		public Invocation parseEvent(LogInfo eventLog) {
			CallTransaction.Function event = getBySignatureHash(eventLog.getTopics()
							.get(0)
							.getData());
			int indexedArg = 1;
			...
			Object[] unindexedArgs = event.decode(eventLog.getData(),
							unindexed.toArray(new Param[unindexed.size()]));
	...
	/**
	 * Represents either function invocation with its arguments or Event instance with its data
	 * members
	 */
	public static class Invocation {
		...
		public String toString() {
			return "[" + "contract=" + contract + (function.type == FunctionType.event ? ", event=" : "," +
              " function=") + function + ", args=" + Arrays.toString(
							args) + ']';
		}
	}
}
