...
import cm.aptoide.pt.web3j.abi.TypeReference.StaticArrayTypeReference;
import cm.aptoide.pt.web3j.abi.datatypes.Array;
import cm.aptoide.pt.web3j.abi.datatypes.Bytes;
import cm.aptoide.pt.web3j.abi.datatypes.BytesType;
...
import cm.aptoide.pt.web3j.abi.datatypes.generated.Bytes32;
import cm.aptoide.pt.web3j.utils.Numeric;
import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import static cm.aptoide.pt.web3j.abi.TypeDecoder.MAX_BYTE_LENGTH_FOR_HEX_STRING;
...
public class FunctionReturnDecoder {
   ...
   */
  @SuppressWarnings("unchecked") public static <T extends Type> Type decodeIndexedValue(
  ...
  private static List<Type> build(String input, List<TypeReference<Type>> outputParameters) {
    ...
    for (TypeReference<?> typeReference : outputParameters) {
      try {
        @SuppressWarnings("unchecked") Class<Type> type =
        ...
        if (DynamicArray.class.isAssignableFrom(type)) {
        ...
        } else if (typeReference instanceof StaticArrayTypeReference) {
          int length = ((StaticArrayTypeReference) typeReference).getSize();
          ...
          int length = Integer.parseInt(type.getSimpleName()
              .substring(StaticArray.class.getSimpleName()
                  .length()));
          result = TypeDecoder.decodeStaticArray(input, hexStringDataOffset, typeReference, length);
          offset += length * MAX_BYTE_LENGTH_FOR_HEX_STRING;
        } else {
